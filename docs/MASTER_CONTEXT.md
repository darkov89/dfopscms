...
- **Wdrożone na Staging (2026-07-05):** `git push origin main:staging` (front), `supabase db push` (obie migracje), `supabase functions deploy record-site-event` + `aggregate-growth-benchmarks`. **Bugfix wykryty przy deployu:** per-funkcyjne `config.toml` (`verify_jwt = false`) w katalogach funkcji **nie wystarcza** w tym CLI — brama JWT Supabase blokowała requesty (`UNAUTHORIZED_INVALID_JWT_FORMAT`) zanim doszły do kodu funkcji. Naprawa: dopisano `[functions.record-site-event]` i `[functions.aggregate-growth-benchmarks]` (`verify_jwt = false`) do **centralnego** `supabase/config.toml` (plik poza .cursorignore — edytowalny tylko przez Shell) + redeploy z `--no-verify-jwt`. Zweryfikowano end-to-end przez `supabase db query --linked`: insert konwersji, `get_page_growth_stats` (zwraca `draft_stale_days`), `aggregate_growth_benchmarks()` (3 motywy zaktualizowane), guard cronu (zły `CRON_SECRET` → 401 z kodu funkcji, nie z bramy).
- **Odwiedziny strony (`page_view`):** migracja `20260705020000_growth_page_views.sql` — nowy `event_scope='visit'` (obok `'conversion'`) w `analytics_events`; RLS `analytics_events_owner_select_conversion` i oba indeksy poszerzone o `event_scope IN ('conversion','visit')`; `get_page_growth_stats()` RPC liczy też `page_view` (ten sam podpis). Edge `record-site-event`: `page_view` w `EVENT_TYPES`, `scopeForEventType()` mapuje na `'visit'`. Front: `publicSiteApp.recordPageView()` woła `DFOPS_recordConversionEvent('page_view','page')` raz w `init()`, **po** bramkach trial-blocked/preview/polityka-prywatności (żeby nie liczyć zablokowanych/podglądowych wejść) i **niezależnie** od bannera zgód cookies (bezcookiesowe, patrz klauzula RODO). Dashboard: 4. kafelek „Odwiedziny (7 dni)” w `tab-dashboard.html` (wyróżniony kolorem accent, grid 2×4). Świadomie: surowe odsłony bez odduplikowania, bez nowej reguły/benchmarku konwersji (decyzja użytkownika 2026-07-05) — do rozważenia w przyszłości.
- **Nie zrobione:** harmonogram cron w Dashboardzie (`aggregate-growth-benchmarks`, np. `0 3 * * 1`, `Authorization: Bearer <CRON_SECRET>`), test manualny G1/G3 na żywym froncie staging.dfcms.pl, `pct_has_offer`/`median_weekly_phone_clicks` niewalidowane na realnych (niedemo) danych, merge `main`→produkcja.
