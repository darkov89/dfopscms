...
- **Subskrypcja panel:** Źródła: `billing_profiles` + lustrzane `pages.billing_plan`; `content.pl.settings.subscription` zawsze trial (strip przy `loadData`). Widok UI: **`billingSubscriptionView`**, **`subscriptionPlan`**, **`hasActivePaidSubscription`** — jawne pola Alpine (nie gettery; Alpine zamraża gettery przy `x-data` init). Ustawiane przez `refreshBillingSubscriptionView()` / `applyBillingSubscriptionView()`. Sync Stripe: toast sukcesu tylko gdy `hasActivePaidSubscription`. **Debug:** `?billing_debug=1` → panel na zakładce Subskrypcja.
