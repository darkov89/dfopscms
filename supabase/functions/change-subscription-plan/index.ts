...
serve(async (req) => {
  ...
  try {
    ...
    if (!targetPriceId) {
      throw new Error(
        "Brak STRIPE_PRICE_PRO / PREMIUM (Secrets) lub priceId w żądaniu.",
      );
    }
    ...
    const curRank = priceTierRank(currentPriceId, priceStarter, pricePro, pricePremium);
    const newRank = priceTierRank(resolvedNewPriceId, priceStarter, pricePro, pricePremium);
