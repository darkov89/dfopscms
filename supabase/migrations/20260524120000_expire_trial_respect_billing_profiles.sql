-- Cron nie może blokować stron z aktywną subskrypcją w billing_profiles
-- ani z pages.billing_plan = tier0/tier1 (źródło prawdy po webhooku Stripe).
...
BEGIN
  WITH trial_targets AS (
    ...
    WHERE p.trial_blocked_at IS NULL
      AND COALESCE(NULLIF(trim(p.billing_plan), ''), 'trial') NOT IN ('tier0', 'tier1')
      AND NOT EXISTS (
        SELECT 1
