...
**Ostatnia aktualizacja treści:** 2026-06-14 — API weryfikacji CNAME własnej domeny (`/api/verify-domain`)
...
| **Custom domains tenantów** | Subdomeny `*.dfcms.pl` wymagają **wildcard Custom Domain** w projekcie Cloudflare Pages (inaczej worker widzi `dfopscms.pages.dev`). Kod ma shim `?site=` + `replaceState`; docelowo dodać `*.dfcms.pl` w Dashboard. **Weryfikacja DNS (CNAME):** `GET /api/verify-domain?domain=…` — Pages Function `functions/api/verify-domain.js` (DoH Cloudflare); akceptowane cele: `proxy.dfcms.pl`, `dfcms.pl`, `dfopscms.pages.dev`. |
...
- **Webhook Stripe** (`supabase/functions/stripe-webhook/`): weryfikacja podpisu przez **`stripe.webhooks.constructEventAsync`** (async, Deno). **wFirma** (`_shared/wfirmaBilling.ts` + **`wfirmaInvoiceLedger.ts`**): po **`checkout.session.completed`** (pierwsza płatność) oraz **`invoice.paid` / `invoice.payment_succeeded`** przy `billing_reason` **`subscription_update`** (upgrade w portalu) lub **`subscription_cycle`** (odnowienie). **Idempotencja:** tabela **`wfirma_invoice_ledger`**. **B2C (bez NIP):** dwukrok — `/contractors/add` → `/invoices/add` z `<contractor><id>…</id></contractor>`; typ **`normal`** + VAT krajowy (czynny płatnik). **B2B:** pełne dane kontrahenta inline. **Zagranica B2B:** **`NPUE`** / **`NP`**. Kontrahent spoza PL → PLN + **pol-ang**. Błąd wFirma **nie** blokuje CMS. Secrets: `WFIRMA_*`.
...
- **`create-checkout`:** body `plan`, `interval`, `returnUrl`; cena z Secrets `STRIPE_PRICE_*` (netto / exclusive w Stripe). **`STRIPE_AUTOMATIC_TAX=true`** (Secret) włącza `automatic_tax` — wymaga Stripe Tax + rejestracji PL w Dashboard (osobno Test/Live). Bez secretu Checkout działa bez doliczania VAT w Stripe. **Returning customer:** odczyt `billing_profiles.stripe_customer_id` → `customer` (bez `customer_email`); nowy klient → `customer_email`. Zawsze `client_reference_id` + `metadata.supabase_user_id`. **wFirma:** netto z `amount_subtotal` / `invoice.subtotal`, brutto z `amount_total` / `amount_paid`.
...
- **Widoczność sekcji (per zakładka + przegląd):** każda sekcja ma toggle włącz/wyłącz **w swojej zakładce** panelu (O nas/Manifesto w Powitaniu, Usługi, Zaufanie, Galeria — **`settings.showGallery`**, Opinie z Google — **`settings.showGoogleReviews`**, FAQ, Kontakt, Opinie własne); lista **„Co widnieje na stronie”** w **Szablon i kolory** pozostaje jako szybki przegląd — te same pola `settings.show*` przez `x-model`, więc zawsze zsynchronizowane (Proof w consultant tylko tam — brak własnej zakładki). Galeria publicznie: `showGallery !== false && images.length` (sekcja + link w nav, beauty/fitness/services). Opinie Google publicznie: `showGoogleReviews !== false && (place_query || place_id || demo)` — wszystkie 4 szablony. Wyjątki bez toggle (celowo): **Powitanie/hero** (fundament strony), **Grafik zajęć** i **Rezerwacje online** — widocznością steruje `settings.booking_mode` (w zakładce Grafiku odnośnik do „Rezerwacje online”), **Wiadomości od gości** (skrzynka w panelu, nie sekcja strony).
- **Smart Booking Module (wszystkie szablony):** klucze **`settings.booking_mode`** (`'schedule' | 'embed' | 'button' | 'both'`) + **`contact.booking_url`** (kanoniczne; legacy `bookingUrl`/`booksyUrl` zsynchronizowane — kontrakt w `contentSchema.js`, migracja w `contentUpgrader`: brak trybu → inferencja Calendly=embed / URL=button / pusty=schedule). **Panel:** osobna zakładka **„Rezerwacje online”** (`activeTab === 'booking'`, wszystkie motywy) — select trybu + warunkowe pole URL (`x-show` przy trybie ≠ schedule) + ostrzeżenie przy embed z Booksy (X-Frame-Options); **`normalizeBookingSettings`** przy load/auto-save/publish; w zakładce Kontakt tylko odnośnik. **Public (`publicSiteApp`):** `bookingMode()`, `bookingModuleActive()`, `showBookingEmbed()`, `showBookingButton()`; embed → iframe (`resolveCalendlyEmbedUrl`), button/both → karta CTA (`var(--color-primary)`), schedule → natywny widok. Sekcje konwersji: beauty/services/consultant `#rezerwacja` (+ legacy `contact.cta.enabled` gdy brak modułu), fitness `#grafik`. **Kontakt/stopka:** wyłącznie mapa, adres, tel, e-mail, social + legal (modal); bez CTA — kompaktowy footer (`pt-10`, bez dużego nagłówka); consultant: `#kontakt` osobno od `#rezerwacja`.
