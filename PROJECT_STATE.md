...
- **`create-checkout`:** body `plan` (`starter` \| `standard`), `interval` (`monthly` \| `yearly`), `returnUrl` — **bez `priceId` z frontu**; cena wyłącznie z Secrets `STRIPE_PRICE_*`. Sesja: `client_reference_id` + `metadata.supabase_user_id` = JWT `user.id`. **409 `HAS_STRIPE_SUBSCRIPTION`** tylko gdy w `billing_profiles` jest żywa sub (`status` active/trialing/past_due/unpaid/paused/incomplete) — po `canceled` / `incomplete_expired` można ponownie Checkout (stare `stripe_subscription_id` w DB nie blokuje).
