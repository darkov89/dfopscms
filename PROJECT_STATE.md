...
**Ostatnia aktualizacja treści:** 2026-05-22 — landing `index.html` #cennik zsynchronizowany z panelem (Starter/Standard/Premium, toggle mies./rok)
...
| **Płatności** | **Stripe** (Checkout, Customer Portal, webhooks → Edge). Pakiety: **Starter**, **Standard**, **Custom** (formularz, bez Checkout). `config.js`: `stripePrices` (`starter`, `starterYearly`, `standard`, `standardYearly`); Secrets: `STRIPE_PRICE_STARTER`, `STRIPE_PRICE_STARTER_YEARLY`, `STRIPE_PRICE_PRO`, `STRIPE_PRICE_PRO_YEARLY`. Legacy `tier2` → `tier1` w kodzie i migracji DB. |
...
- **Cykle rozliczeniowe (Starter, Standard):** przełącznik **miesięcznie / rocznie (−20%)**; Checkout: `interval` + `plan: starter|standard`. Webhook/sync mapują ceny roczne na `tier0` / `tier1`.
