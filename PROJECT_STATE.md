...
**Ostatnia aktualizacja treści:** 2026-06-05 — wFirma cross-border B2B (VAT UE, stawka NP); porządek repo
...
- **Webhook Stripe** (`supabase/functions/stripe-webhook/`): weryfikacja podpisu przez **`stripe.webhooks.constructEventAsync`** (async, Deno). **wFirma** (`_shared/wfirmaBilling.ts`): po **`checkout.session.completed`** (pierwsza płatność) oraz **`invoice.paid` / `invoice.payment_succeeded`** przy `billing_reason` **`subscription_update`** (upgrade w portalu) lub **`subscription_cycle`** (odnowienie). B2B PL: `tax_id_type=nip` + 10 cyfr; B2B UE: `tax_id_type=eu_vat` + pełny VAT (np. `DE…`) + stawka **`np`**. B2C: `bill` + **`zw`**. Błąd wFirma **nie** blokuje CMS. Secrets: `WFIRMA_*`.
