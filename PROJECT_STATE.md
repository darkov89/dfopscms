...
**Ostatnia aktualizacja treści:** 2026-05-23 — rozliczenia (mies./rok, Starter/Standard/Custom) + lifting UI panelu (sidebar, top bar, karty Subskrypcja)
...
- **`create-checkout`:** body `plan` (`starter` \| `standard`, alias `pro`), `interval` (`monthly` \| `yearly`), opcjonalnie `priceId` z `config.js`; metadata `billing_interval`, `plan`.
