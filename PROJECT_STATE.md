...
**Ostatnia aktualizacja treści:** 2026-06-05 — `stripe-webhook` + wFirma (faktura e-mail po Checkout, opcjonalne Secrets)
...
| **Panel CMS** | `admin.html`, **Alpine.js**, **Tailwind** (CDN), `js/features/adminApp.js`. **Monitoring:** Sentry Loader Script wpięty jako **pierwszy** element `<head>` w `admin.html` (bez `Sentry.init()`). **Toast (global):** nieblokujące, auto-dismiss **3s**, success zielony / error czerwony. **Confirm (global):** systemowy `confirm()` zastąpiony modalem **Promise<boolean>** (`confirmAsync` + `await`) w krytycznych akcjach (usuwanie konta, zmiana szablonu, domena, odrzucenie zmian). **Publikacja:** przycisk „Publikuj zmiany” ma dodatkowe **pozytywne tarcie** (modal potwierdzenia), ale logika `publishChanges()` (draft→content) bez zmian; końcówka kreatora i flow domeny publikują bez tego modala. **Podgląd vs Live:** link „Podgląd strony” otwiera draft, a klikalny slug w top-barze otwiera wersję **LIVE** (custom domain jeśli aktywna, inaczej subdomena). **Subskrypcja UI:** jedna biała karta statusu + sekcja „Zarządzanie rozliczeniami” w stylu admina; akcje nadal otwierają odpowiednie kroki w **Stripe Customer Portal** (badge „Obsługa: Stripe”). Pozostałe elementy: cennik z `billingInterval`, plany Starter/Standard/Custom, Warunki rozliczeń w `<details>`, sync ze Stripe, itd. |
...
2. **Rejestracja** — `rejestracja.html` + **`registrationApp.js`**: kolejność w DB **najpierw** `auth.users` (`signUp`), **potem** `pages` (trigger **`handle_new_user`** po INSERT, migracja **`20260525130000`** — kolizja slug → rollback całej rejestracji; trial w `content`). Slug tylko w **`user_metadata`**, nie insert z przeglądarki (RLS). Przy **Confirm email** Supabase często zwraca **`user: null` bez błędu** (ten sam e-mail) — front: jeśli slug już zajęty → sukces „sprawdź skrzynkę”, nie „nie udało się utworzyć użytkownika”. Fallback strony: **`adminApp.ensurePageFromRegistrationMetadata`** przy pierwszym logowaniu.  
