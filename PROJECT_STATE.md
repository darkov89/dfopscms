...
- **Webhook Stripe** (`supabase/functions/stripe-webhook/`): weryfikacja podpisu przez **`stripe.webhooks.constructEventAsync`** (async, Deno). Po **`checkout.session.completed`**: opcjonalnie **wFirma** (`_shared/wfirmaBilling.ts`) — B2B PL (VAT 23%), B2B UE (WDT, stawka **NP**, pełny numer VAT z prefiksem kraju), B2C (`bill`, **zw**); wysyłka `/invoices/send`; błąd wFirma **nie** blokuje zapisu w Supabase. Secrets: `WFIRMA_ACCESS_KEY`, `WFIRMA_SECRET_KEY`, `WFIRMA_APP_KEY`, opcj. `WFIRMA_COMPANY_ID`.
